...
public class Frequencer implements FrequencerInterface{
    ...
    private int suffixCompare(int i, int j) {
        ...
        // ここにコードを記述せよ 
        //             
	int x=mySpace.length-i;
	int y=mySpace.length-j;
	...
	for(int n=0;n<mySpace.length-1;n++){
		for(int m=mySpace.length-1;m>n;m--){
			int temp;
			if(suffixCompare(suffixArray[m-1],suffixArray[m])==1){
				temp=suffixArray[m-1];
    ...
    public static void main(String[] args) {
	    ...
	    //System.out.println(frequencerObject.suffixCompare(3,2));
